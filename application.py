...
i8 = conv1[0].T.sum(axis=0)
i9 = conv2[0].T.sum(axis=0)
i10 = conv3[0].T.sum(axis=0)
i11 = conv4[0].T.sum(axis=0)
i12 = conv5[0].T.sum(axis=0)
...
dataset = range(0, TOTAL_VIDEOS - 1)
...
for q in query_videos:
    ...
    for nearest_neighbour in y:
        ...
        try:
            test_vids = test_vids | inverted_index[nearest_neighbour] # set 에서의 or 연산은 더학임!
        except Exception as e:
            print(e)
